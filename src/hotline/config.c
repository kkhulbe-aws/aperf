...
void parse_arguments(int argc, char *argv[]) {
  static struct option long_options[] = {{"wakeup_period", required_argument, 0, 'p'},
                                         {"hotline_sample_frequency", required_argument, 0, 's'},
  ...
  profile_configuration.hotline_sample_frequency = PROFILE_DEFAULT_SPE_SAMPLE_FREQ;
  profile_configuration.timeout = PROFILE_DEFAULT_TIMEOUT;
  profile_configuration.data_dir = "./data";
  profile_configuration.report_dir = "./report";
  ...
  while ((c = getopt_long_only(argc, argv, "", long_options, &option_index)) != -1) {
    switch (c) {
      case 'p':
        ...
        break;
      case 's':
        ASSERT(sscanf(optarg, "%" SCNu32, &profile_configuration.hotline_sample_frequency) == 1,
      ...
      case 'd':
        ...
        break;
      case 'r':
        profile_configuration.report_dir = optarg;
        break;
      case 'n':
      ...
      case '?':
        ...
        printf(
            "Usage: ./<BINARY> --wakeup_period X --hotline_sample_frequency X "
            "--timeout X "
  ...
  ASSERT(profile_configuration.hotline_sample_frequency > 0 &&
             profile_configuration.hotline_sample_frequency < MAX_SPE_SAMPLE_FREQ,
...
void get_perf_buffer_sizes(perf_buffer_size_t *buffer_sizes) {
  ...
  uint64_t perf_aux_buf_sz = profile_configuration.hotline_sample_frequency *
