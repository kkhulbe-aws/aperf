...
bmiss_map_entry_t *deserialize_bmiss_map(const char *filename, uint64_t *out_count) {
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    int result =
        sscanf(line, "%[^,],%lx,%lu,%lu,%lu,%lu,%lu,%lu,%hhx", temp_filename, &entry->offset,
               &entry->retired, &entry->not_taken, &entry->mispredicted, &entry->total_latency,
...
lat_map_entry_t *deserialize_lat_map(const char *filename, uint64_t *out_count) {
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    int result = sscanf(line,
                        ...
                        "%ld",
                        temp_filename, &entry->offset, &entry->retired, &entry->total_latency,
...
void generate_bmiss_report(const char *file_dir, bmiss_map_entry_t *entries, uint64_t count) {
  ...
  fprintf(fp,
          ...
          "Saturated,Location,Line Number,Line,Function,Assembly\n");
  ...
  for (size_t i = 0; i < MIN(count, profile_configuration.num_to_report); i++) {
    ...
    fprintf(fp, "%s,%ld,%.2f,%.2f,%ld,%ld,%ld,%s,%ld,%s,%s,%s\n",
            entries[i].branch_type == 0x01 ? "COND" : "IND", entries[i].retired,
            ((double)entries[i].total_latency / entries[i].retired),
            ((double)entries[i].issue_latency / entries[i].retired), entries[i].not_taken,
            entries[i].mispredicted, entries[i].saturated, dinfo->src_file, dinfo->line_num,
...
int compare_lat_exec_entries(const void *a, const void *b) {
  ...
  return (entry_b->total_latency - entry_b->issue_latency - entry_b->translation_latency) -
         (entry_a->total_latency - entry_b->issue_latency - entry_b->translation_latency);
}
...
void print_issue_latency(FILE *fp, const lat_map_entry_t *entry, const debug_info_t *dinfo) {
  fprintf(fp, "%.2f,%ld,%s,%ld,%s,%s,%s,%ld\n", (double)(entry->issue_latency) / entry->retired,
          entry->retired, dinfo->src_file, dinfo->line_num, dinfo->line, dinfo->function,
          dinfo->assembly, entry->saturated);
}
...
void print_translation_latency(FILE *fp, const lat_map_entry_t *entry, const debug_info_t *dinfo) {
  fprintf(fp, "%.2f,%ld,%s,%ld,%s,%s,%s,%ld\n",
          (double)(entry->translation_latency) / entry->retired, entry->retired, dinfo->src_file,
          dinfo->line_num, dinfo->line, dinfo->function, dinfo->assembly, entry->saturated);
}
...
void generate_lat_report(const char *file_dir, lat_map_entry_t *entries, uint64_t count) {
  ...
  fputs(
      "Avg. Exec Latency,Count,Location,Line "
      "Number,Line,Function,Assembly,Saturated\n",
      exec_fp);

  fputs(
      "Avg. Issue Latency,Count,Location,Line "
      "Number,Line,Function,Assembly,Saturated\n",
      issue_fp);
