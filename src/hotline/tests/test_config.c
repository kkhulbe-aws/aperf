...
void test_parse_arguments_defaults() {
  ...
  assert(profile_configuration.hotline_sample_frequency == PROFILE_DEFAULT_SPE_SAMPLE_FREQ);
...
void test_parse_arguments_custom() {
  ...
  char *argv[] = {"test_program", "--wakeup_period", "5",          "--hotline_sample_frequency",
                  "2000",         "--timeout",       "30",         "--data_dir",
                  "/tmp/data",    "--report_dir",    "/tmp/report"};
  int argc = 11;

  parse_arguments(argc, argv);

  assert(profile_configuration.wakeup_period == 5);
  assert(profile_configuration.hotline_sample_frequency == 2000);
...
void test_get_perf_buffer_sizes() {
  ...
  profile_configuration.hotline_sample_frequency = 1000;
  ...
  uint64_t expected_aux_buf_raw = 1000 * 2 * sizeof(spe_record_raw_t) * 8;
  ...
  assert((buffer_sizes.perf_aux_buf_sz & (buffer_sizes.perf_aux_buf_sz - 1)) ==
         0);  // power of 2 check
}

void test_get_perf_buffer_sizes_different_config() {
  profile_configuration.wakeup_period = 1;
  profile_configuration.hotline_sample_frequency = 500;
