...
void init_perf_hardware_event(cpu_session_t *session) {
  ...
  attr.sample_period = cpu_system_config.frequency / profile_configuration.hotline_sample_frequency;
...
void init_perf_software_event(cpu_session_t *session) {
  ...
  fd = perf_event_open(&attr, -1, session->cpu, session->hardware_fd, PERF_FLAG_FD_CLOEXEC);
...
void serialize_bmiss_map() {
  ...
  while (ok) {
    ...
    int write_result =
        fprintf(branch_fp, "%s,0x%lx,%lu,%lu,%lu,%lu,%lu,%lu,%x\n", finode_entry->filename,
                entry->offset, entry->retired, entry->not_taken, entry->mispredicted,
                entry->total_latency, entry->issue_latency, entry->saturated, entry->branch_type);
  ...
  while (ok) {
    ...
    int write_result =
        fprintf(lat_fp,
                ...
                "%lu,%lu,%lu,%lu,"        // l3 bins
                "%lu,%lu,%lu,%lu,%lu\n",  // dram bins
                finode_entry->filename, entry->offset, entry->retired, entry->total_latency,
...
void handle_signal(int signum __attribute__((unused))) { terminate_flag = 1; }
