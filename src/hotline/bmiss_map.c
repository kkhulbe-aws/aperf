...
inline void parse_bmiss_map_entry(spe_record_raw_t *record, bmiss_map_entry_t *entry,
  ...
  // Don't update statistics if saturated, because the cycle counts cannot be
  // trusted and can cause misleading statistics. Instead log it and report
  // saturated counts.
  if (entry->saturated) return;

  entry->total_latency = record->total_lat;
  entry->issue_latency = record->issue_lat;
  entry->not_taken = (record->events_packet & AUX_EVENT_BRANCH_NOT_TAKEN) ? 1 : 0;
