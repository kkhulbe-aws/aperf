...
void commit_to_file() {
  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/%s", get_hotline_dir(),
           "spe_hotline_loads.bin");
  FILE *load_fp = fopen(path, "w");

  memset(path, 0, sizeof(char) * PATH_MAX);
  snprintf(path, sizeof(path), "%s/%s", get_hotline_dir(),
           "spe_hotline_branches.bin");
  ...
  // write headers
  fprintf(load_fp, "filename,offset,retired_insts,total_latency,issue_latency,"
                   "translation_latency,"
  ...
  fprintf(branch_fp,
          "filename,offset,retired_insts,not_taken_branches,mispredicted,total_"
          "latency,issue_latency,saturated,branch_type\n");
  ...
  while (ok) {
    ...
    if (entry->type == AUX_RECORD_LOAD) { // LOAD
      fprintf(load_fp,
              "%s,0x%lx,%lu,%lu,%lu,%lu,"
              "%lu,%lu,%lu,%lu,"       // l1 bins
              "%lu,%lu,%lu,%lu,"       // l2 bins
