...
{
    const struct vm_spe_btree_entry *a_entry = (const struct vm_spe_btree_entry *)a;
    const struct vm_spe_btree_entry *b_entry = (const struct vm_spe_btree_entry *)b;

    // bare minimum comparisons required to uniquely identify a VA to map SPE records to
