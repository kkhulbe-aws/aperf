                         ...
                         const struct mmap2_mapping *record)
{
    ...
    {
        maps->capacity *= 2;
        maps->maps = realloc(maps->maps,
                             maps->capacity * sizeof(struct map_entry));
    }
...
int pc_to_file_offset(struct pid_maps *maps, uint64_t pc,
        ...
        while (maps)
        {
            ...
            for (size_t j = 0; j < maps->count; j++)
            {
                ...
                printf("  0x%lx - 0x%lx : %s (file offset: 0x%lx)\n",
                       ...
                       global_filenames[entry->filename_index], entry->pgoff);
            }
            maps = maps->next;
            if (maps)
            {
...
void get_initial_mappings(struct pid_maps_table *table)
{
    ...
    while ((pid_entry = readdir(proc_dir)))
    {
        ...
        snprintf(maps_path, sizeof(maps_path),
                 "/proc/%.230s/maps", pid_entry->d_name);
        ...
        while (fgets(line, sizeof(line), maps))
        {
            ...
            sscanf(line, "%lx-%lx %4s %lx %*x:%*x %*u %255s",
...
size_t get_pid_maps_table_size(struct pid_maps_table *table)
{
    ...
    total_size += sizeof(char *) * MAX_FILENAMES; // size of global_filenames array
