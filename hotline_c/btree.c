...
static size_t btree_align_size(size_t size)
{
    ...
    return size < boundary ? boundary : size & (boundary - 1) ? size + boundary - (size & (boundary - 1))
                                                              : size;
}
    ...
    void *udata)
{
    ...
    malloc_ = malloc_ ? malloc_ : _btree_malloc ? _btree_malloc
                                                : malloc;
    free_ = free_ ? free_ : _btree_free ? _btree_free
                                        : free;
