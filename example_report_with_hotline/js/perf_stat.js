...
let perf_stat_rules = {
    ...
    rules: [
        {
            name: "ipc",
            per_run_rule: function* (ruleOpts) {
        ...
        },
        {
            ...
            single_run_rule: function* (opts) {
                ...
                let thresh = 20.;
                if (l1_mpki < thresh) {
                    yield new Finding(`L1 MPKI for '${opts.base_run}' is less than ${thresh}`, Status.Good);
                }
                else {
                    yield new Finding(`L1 MPKI for '${opts.base_run}' is greater than ${thresh}`, Status.NotGood);
                }
            },
        },
        {
            ...
            single_run_rule: function* (opts) {
                ...
                let thresh = 10.;
                if (l2_mpki < thresh) {
                    yield new Finding(`L2 MPKI for '${opts.base_run}' is less than ${thresh}`, Status.Good);
                }
                else {
                    yield new Finding(`L2 MPKI for '${opts.base_run}' is greater than ${thresh}`, Status.NotGood);
                }
            },
        },
        {
            ...
            single_run_rule: function* (opts) {
                ...
                let thresh = 2.;
                if (l3_mpki < thresh) {
                    yield new Finding(`L3 MPKI for '${opts.base_run}' is less than ${thresh}`, Status.Good);
                }
                else {
                    yield new Finding(`L3 MPKI for '${opts.base_run}' is greater than ${thresh}`, Status.NotGood);
                }
            },
        }
    ]
