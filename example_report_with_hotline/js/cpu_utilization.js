...
let cpu_utilization_rules = {
    ...
    rules: [
        {
            name: "User",
            single_run_rule: function* (opts) {
                let system_util = get_data_key(opts.data_type, "System");
                let total_util = opts.base_run_data + system_util.get(opts.base_run);
                if (total_util < 50) {
                    yield new Finding(`Average CPU Utilization for '${opts.base_run}' is less than 50%.`, Status.NotGood);
                }
            },
            per_run_rule: function* (opts) {
                let system_util = get_data_key(opts.data_type, "System");
                let init_total_util = opts.base_run_data + system_util.get(opts.base_run);
                let run_total_util = opts.this_run_data + system_util.get(opts.this_run);
                let cpu_diff = Math.ceil(Math.abs(run_total_util - init_total_util));
                yield new Finding(`Average CPU Utilization difference between '${opts.base_run}' and '${opts.this_run}' is ${cpu_diff}%.`, cpu_diff > 10 ? Status.NotGood : Status.Good);
            },
        },
        {
            name: "Idle",
            ...
            per_run_rule: function* (opts) {
                ...
                yield new Finding(`Average Idle time difference between '${opts.base_run}' and '${opts.this_run}' is ${idle_diff}%.`, idle_diff > 10 ? Status.NotGood : Status.Good);
            },
        }
    ]
};
