...
let system_info_rules = {
    ...
    rules: [
        {
            name: "System Name",
            all_run_rule: function* (ruleOpts) {
        ...
        for (var j = 0; j < rules_group.rules.length; j++) {
            ...
            if (runs_raw.length > 1) {
                for (const [key, value] of opts.other_run_data) {
                    ...
                    let gen = (_a = rule.per_run_rule) === null || _a === void 0 ? void 0 : _a.call(rule, opts);
                    if (gen) {
                        let result = gen.next();
                        while (!result.done) {
                ...
                let gen = (_b = rule.all_run_rule) === null || _b === void 0 ? void 0 : _b.call(rule, opts);
                if (gen) {
                    let result = gen.next();
                    while (!result.done) {
                        analytics.analysis = analytics.analysis.concat(result.value);
            ...
            for (var k = 0; k < runs_raw.length; k++) {
                ...
                let gen = (_c = rule.single_run_rule) === null || _c === void 0 ? void 0 : _c.call(rule, opts);
                if (gen) {
                    let result = gen.next();
                    while (!result.done) {
                        analytics.analysis = analytics.analysis.concat(result.value);
