...
function openData(evt, elem) {
    var tabName = elem.name;
...
DataTypes.forEach((datatype, key) => {
    ...
    for (var j = 0; j < button_elems.length; j++) {
        button_elems[j].addEventListener("click", function (evn) {
    ...
    for (var j = 0; j < select_elems.length; j++) {
        select_elems[j].addEventListener("click", function (evn) {
            datatype.callback(this.id);
        });
    }
    if (runs_raw.length == 1) {
        if (datatype.onSingleRun != '') {
            document.getElementById(`${datatype.name}-button-${datatype.onSingleRun}`).checked = true;
        }
    }
});
var run_width = 100;
